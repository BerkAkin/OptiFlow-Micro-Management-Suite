...
function PackCard({ Header, HeaderBg, ButtonText, Price, SingleItems, Text, Height, Width, IsHover }: PackCardProps) {
    return (
        <>
            ...
            <div className={`border border-gray-200 bg-white mx-4 w-${Width} h-[${Height}px] ${IsHover ? "hover:scale-105" : ""} rounded-lg shadow-lg hover:ease-in transition duration-150`}>
                <div className={`bg-${HeaderBg} grid grid-cols-[75%_25%] rounded-t-lg`} style={{ height: (Height * 15 / 100) }}>
                    ...
                    <div className='flex items-center justify-center border-s border-gray-200'><p className='text-white text-3xl'>${Price}</p></div>
                </div>
                ...
                <div className='border-t border-gray-200 border-b ps-5' style={{ height: (Height * 45 / 100) }}>
